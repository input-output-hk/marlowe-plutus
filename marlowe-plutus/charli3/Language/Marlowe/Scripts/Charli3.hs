...
mkValidator
  ...
  ScriptContext{scriptContextTxInfo = TxInfo{..}, scriptContextPurpose} = do
    let ownInput =
        ...
        (charli3Value, PV2.Datum charli3Datum) =
          case txInfoReferenceInputs of
            [TxInInfo{txInInfoResolved = TxOut{txOutValue, txOutDatum = OutputDatum datum}}, _] -> (txOutValue, datum)
            [_, TxInInfo{txInInfoResolved = TxOut{txOutValue, txOutDatum = OutputDatum datum}}] -> (txOutValue, datum)
            _ -> traceError "C3d"
        charli3Oracle =
        ...
        marloweInput = case find (\TxInInfo{txInInfoResolved} -> txOutAddress txInInfoResolved == marloweValidatorAddress) txInfoInputs of
          Just input -> input
          Nothing -> traceError "C3g"

        threadTokenOk = do
          let marloweValue = txOutValue $ txInInfoResolved marloweInput
          traceIfFalse "C3h" (valueOf marloweValue currencySymbol threadTokenName > 0)
        ...
        marloweRedeemerOk = do
          let TxInInfo{txInInfoOutRef = marloweTxOutRef} = marloweInput
              ...
              inputs = case AssocMap.lookup (Spending marloweTxOutRef) txInfoRedeemers of
                Nothing -> traceError "C3i"
                Just (Redeemer bytes) -> case fromBuiltinData bytes of
                  Just inputs -> inputs
                  Nothing -> traceError "C3j"
          isJust $ find inputUsesRole inputs

    ownDatumOk && charli3ValueOk && threadTokenOk && marloweRedeemerOk
